/*
  TCA9554.h - Library for work with TCA9554 or TCA9554a GPIO expander
  Created by Prohorov Vladimir, Octomber 14, 2019.
  Released into the public domain.
*/
#ifndef TCA9554_H
#define TCA9554_H


#include "Arduino.h"
#include <Wire.h>

#define INPUT_PORT_REG 0x00
#define OUTPUT_PORT_REG 0x01
#define POLARITY_INVERSION_REG 0x02
#define CONFIG_REG 0x03

class TCA9554
{
  public:
    TCA9554(int address);
    int PinSetMode(byte pin, bool mode);
    int DigitalWrite(byte pin, bool state);   // it return 0 if write successful, else retrun -1
    int DigitalRead(byte pin);                // it return pin state if read successful, else return -1
    int GetAddress();                         // it return init address
    int PinState();      
    int PinMode();
    int WriteAll(byte value); 
    int SetLamps(byte lamps[8]);
    int AllOn();
    int AllOff();                       
  private:
    int _address;
};

TCA9554::TCA9554(int address)
{
  _address = address;
}

int TCA9554::PinSetMode(byte pin, bool mode)
{
  byte output_reg = 0;

  Wire.beginTransmission(_address);
  Wire.write(CONFIG_REG);
    
  Wire.requestFrom(_address, 1);
  while (Wire.available()) 
  {                                   
    char output_reg = Wire.read();    
  }
  
  byte error = Wire.endTransmission();
  if (error > 0)
    return -1;

  if (mode == INPUT)
  {
    output_reg |= (1 << pin);
  }
  else if (mode == OUTPUT)
  {
    output_reg &= ~(1 << pin);
  }
  
  Wire.beginTransmission(_address);
  Wire.write(CONFIG_REG);
  Wire.write(output_reg);
  error = Wire.endTransmission();

  if (error > 0)
    return -1;

  return 0;
}

int TCA9554::DigitalWrite(byte pin, bool state)
{
  byte output_reg = 0;
  // This code get current register data
  Wire.beginTransmission(_address);
  Wire.write(OUTPUT_PORT_REG);
    
  Wire.requestFrom(_address, 1);
  while (Wire.available()) 
  {                                   
    char output_reg = Wire.read();    
  }
  
  byte error = Wire.endTransmission();
  if (error > 0)
    return -1;

  // This code get update register 
  if (state == HIGH)
  {
    output_reg |= (1 << pin);
  }
  else if (state == LOW)
  {
    output_reg &= ~(1 << pin);
  }
  output_reg = ~output_reg;

  Wire.beginTransmission(_address);
  Wire.write(OUTPUT_PORT_REG);
  Wire.write(output_reg);
  error = Wire.endTransmission();

  if (error > 0)
    return -1;

  return 0;
}

int TCA9554::DigitalRead(byte pin)
{
  byte output_reg = 0;
  // This code get current register data
  Wire.beginTransmission(_address);
  Wire.write(INPUT_PORT_REG);
    
  Wire.requestFrom(_address, 1);
  while (Wire.available()) 
  {                                   
    char output_reg = Wire.read();    
  }
  
  byte error = Wire.endTransmission();
  if (error > 0)
    return -1;
  
  if ((output_reg & (1 << pin)) > 0)
    return 1;
  else
    return 0;  
}

int TCA9554::PinMode() {
    byte output_reg = 0;
  // This code get current register data
  Wire.beginTransmission(_address);
  Wire.write(OUTPUT_PORT_REG);
    
  Wire.requestFrom(_address, 1);
  while (Wire.available()) 
  {                                   
    char output_reg = Wire.read();    
  }
  
  byte error = Wire.endTransmission();
  if (error > 0)
    return -1;
  
  Wire.beginTransmission(_address);
  Wire.write(CONFIG_REG);
  Wire.write(0x00);
  error = Wire.endTransmission();

  if (error > 0)
    return -1;

  return 0;
}

int TCA9554::PinState() {
    byte output_reg = 0;
  // This code get current register data
  Wire.beginTransmission(_address);
  Wire.write(OUTPUT_PORT_REG);
    
  Wire.requestFrom(_address, 1);
  while (Wire.available()) 
  {                                   
    char output_reg = Wire.read();    
  }
  
  byte error = Wire.endTransmission();
  if (error > 0)
    return -1;
  
  Wire.beginTransmission(_address);
  Wire.write(OUTPUT_PORT_REG);
  Wire.write(0xFF);
  error = Wire.endTransmission();

  if (error > 0)
    return -1;

  return 0;
}

int TCA9554::SetLamps(byte lamps[8])
{
  byte value = 0xFF;  // all OFF (active high = off)
  for (byte i = 0; i < 8; i++) {
    if (lamps[i] == 1) {
      value &= ~(1 << i);  // active-low ON
    }
  }
  return WriteAll(value);
}

int TCA9554::WriteAll(byte value)
{
  Wire.beginTransmission(_address);
  Wire.write(OUTPUT_PORT_REG);
  Wire.write(value);
  byte error = Wire.endTransmission();
  if (error > 0)
    return -1;
  return 0;
}

int TCA9554::AllOff() {
  Wire.beginTransmission(_address);
  Wire.write(OUTPUT_PORT_REG); // select output register
  Wire.write(0xff);            // all pins LOW (OFF)
  byte error = Wire.endTransmission();
  if (error > 0)
    return -1;
  return 0;
}

int TCA9554::AllOn() {
  Wire.beginTransmission(_address);
  Wire.write(OUTPUT_PORT_REG); // select output register
  Wire.write(0x00);            // all pins LOW (OFF)
  byte error = Wire.endTransmission();
  if (error > 0)
    return -1;
  return 0;
}

int TCA9554::GetAddress()
{
  return _address;
}
#endif
